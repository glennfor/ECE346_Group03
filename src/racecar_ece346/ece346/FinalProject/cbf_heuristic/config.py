...
DEFAULT_CONFIG_PATH = Path(__file__).with_name("cbf_heuristic.yaml")
...
def declare_and_load(node: Any, names: Iterable[str] = None) -> SafetyFilterParams:
    node.declare_parameter("cbf_heuristic_config", str(DEFAULT_CONFIG_PATH))
    config_path = str(node.get_parameter("cbf_heuristic_config").value)
