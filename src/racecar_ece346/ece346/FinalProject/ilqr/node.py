"""ROS2 node for the ILQR-QP safety filter."""
...
from .config import DEFAULT_CONFIG_PATH, IlqrQpConfig, load_config
from .filter import FilterCommand, IlqrQpFilter
from .geometry import obstacles_from_msg, path_from_msg, state_from_odom


class IlqrQpSafetyFilterNode(Node):
    def __init__(self):
        ...
        self.filter = IlqrQpFilter(self.config)
        ...
        self.get_logger().info(
            "ilqr_qp safety_filter_node ready: "
    ...
    def _tick(self):
        ...
        if self._is_stale(self._teleop_time, self.config.teleop_timeout_sec):
            self._publish_drive(0.0, 0.0, self._latest_teleop)
        ...
        command = self.filter.filter_command(
            ...
            state=state_from_odom(self._latest_odom, self._last_steering),
